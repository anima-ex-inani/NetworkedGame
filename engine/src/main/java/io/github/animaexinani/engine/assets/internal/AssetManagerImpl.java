...
public final class AssetManagerImpl extends AssetManager {
    ...
    private void cleanAssetCache() {
        var ref = (KeyedSoftReference<? extends Asset>)assetCacheReferenceQueue.poll();

        while (Objects.nonNull(ref)) {
            var key = ref.key;
            assetCache.remove(key, ref);
            ref = (KeyedSoftReference<? extends Asset>)assetCacheReferenceQueue.poll();
        }
    }

    private <T extends Asset> void cacheAsset(@NotNull AssetKey<T> key, @NotNull T asset) {
        if (assetCacheWrites.getAndIncrement() % ASSET_CACHE_WRITE_THRESHOLD == 0) {
            cleanAssetCache();
        }

        assetCache.put(key, new KeyedSoftReference<>(asset, key, assetCacheReferenceQueue));
    }
    ...
    private <T extends Asset> @NotNull Future<T> loadAsset(@NotNull AssetKey<T> key, @NotNull AssetLoadingContextImpl context) {
        ...
        fetchAsset(key, context).whenComplete((asset, throwable) -> {
            ...
            pendingAssetLoads.remove(key, future);
        });
    ...
    private <T extends Asset> @Nullable T getCachedAsset(@NotNull AssetKey<T> key) {
        @SuppressWarnings("unchecked") var assetReference = (KeyedSoftReference<T>)assetCache.get(key);
        ...
        if (Objects.isNull(asset)) {
            assetCache.remove(key, assetReference);
            return null;
        }
    ...
    public <T extends Asset> Future<T> load(@NotNull AssetKey<T> key) {
        var cachedAsset = getCachedAsset(key);
    ...
    public <T extends Asset> Future<T> loadSubasset(@NotNull AssetKey<T> key, @NotNull AssetLoadingContextImpl context) {
        ...
        return loadAsset(key, newContext);
    }
}
